var argos = (function() {
	...
	this.toggleContactPreferences = function() { 			
				//$('#contactPreferencePolicy').replaceWith($('#contactPreferences').html());
				//$('#contactPreferencePolicy').removeClass("showSection");
				//$('#contactPreferencePolicy').addClass("hideSection");
				$('#contactPreferences').removeClass("hideSection");
				$('#contactPreferences').addClass("showSection");
	};
	...
	this.messages = new (function() {
		...
		this.createActivators = function(context) {
			// Find and activate MessageActivators in passed context (no argumetns will result in document context).
			argos.messages.addToHtml($("[class*='messageActivatee']", context));
			$("[class*='messageActivator'], .contextInformation", context).each(function() {
				new argos.classes.MessageActivator(this, argos.page.elements.messageActivatee);
			});
		}
	...
	this.url = new (function() {
		...
		// Changes "http://hostname/path" and "/path" URLs to "https://hostname/path" if in HTTPS
		this.protocolChecked = function(url) {
			return argos.page.isSecure() ? argos.utils.secureUrl(url) : url;
		}

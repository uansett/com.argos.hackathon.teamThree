...
/* 24/01/2013 PM Temporary fix for missing events (scView and scOpen) in s.events */
/*24/06/2013 MM - Installed new plugin - Form Analysis 2.2  and set up with mobile registration form
	03/07/2013 	MM: had to remove registerUserFormMobile from Form Analysis as plugin doesn't support 
								multiple forms on the same page.
	04/07/2013 	MM: added new line that copies eVar5 to prop10, if eVar5 has value. 
								This is to copy the internal campaign id to prop10.

/* var s_account="argosprod,argosprodv14" */

s_account=s_account.replace(",argosglobalprod","");
s_account=s_account.replace("argosprod","argosprod");
...
function s_doPlugins(s) {
	...
	//mobile web visitor id pickup code
	var appVid = s.getQueryParam('appvi');
	if(!appVid){
		appVid = s.c_r('NativeAppVID');
	}
	if (appVid) {
		s.new_vi_date=new Date;
		s.new_vi_date.setFullYear(s.new_vi_date.getFullYear() + 5);
		s.c_w('app_vi',appVid,s.new_vi_date);
	...
	//Passing ADO Visitor ID to SiteCatalyst
	s.eVar45="";
	...
	if(s.pageName){
		...
		//during checkout, if we go straight to step2 set sevents to sccheckout 
		var prevVal = s.getPreviousValue(s.pageName,'prevPageName');
		if (s.pageName == "ar:trolley:stockavailability:"){
				if (prevVal != "ar:trolley:findastore:" && ( (s.events.indexOf('event33')<0) && (s.events.indexOf('event57')<0) && (s.events.indexOf('event7')<0) ) ){
					s.events="scCheckout";
				}
		}
	...
	if(s.prop7 == null) { //MM: Checks to see if not already been set in backend
	...
	s.eVar7=s.prop7;
	if(s.prop7=='Registered' && s.events.match(/event5/g)==null){
	       s.events=s.events+",event5";
	}
...
/* Copying prop9 (WebsphereID) into transactionID */
	...
	/* Temporary fix to add scView if it is the cart page */
	if (s.pageName == 'ar:trolley:trolleylist:') {
		s.events = s.apl(s.events,'scView',',',2);
	}

	/* Temporary fix to add scOpen together with scAdd */
	if (s.inList('scAdd',s.events,',')) {
		s.events = s.apl(s.events,'scOpen',',',2);
		if (typeof(s.lnk) !== "undefined") {
...
/*
	Form Analysis Config for Form Analysis 2.2 Plugin
*/

s.formList=""
s.trackFormList = true; //Set to true to only track listed forms
s.trackPageName = true;
s.useCommerce=false;
s.varUsed="prop41";
s.eventList=""; //

//MM: (04/07/2013) s.prop10 set to s.eVar5 if s.eVar5 is set
// This is the internal campaign ID.
...
/*
 * Plugin: Form Analysis 2.2 (Success, Error, Abandonment) - MM June 2013
 */

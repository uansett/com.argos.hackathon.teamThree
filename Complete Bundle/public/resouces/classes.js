...
argos.classes = new (function() {
	...
	var _helpers = {
		...
		dateFromString : argos.utils.dateFromString,
	...
	CheckStockActivator.prototype.intent = function(e) {
        ...
        if(location) {
            ...
            // Close Activatee and reveal One-Click;
            argos.pdp.fulfilment.storeFound(activatee.getStore());
            activatee.close();
        }
	}
	CheckStockActivator.prototype.getLocation = function() {
		 var $form = this.$node.parents("form");
	        var input = $("#stock01_OOPS", $form).val();
	        var location;
			var $node = this.$node;
			var activatorId = this.$node.attr("id");
			var tagName = this.node.tagName.toLowerCase();
			var isEmailLink = this.$node.hasClass("emailMe") ? true : false;
	       	var params = "";
			switch (tagName) {
				case "a" : 
					if (isEmailLink) {
        ...
        switch(this.node.id) {
            case "findStock": location = this.$node.attr("href");
                                break;
                             
          /*  case "checkStock_go" : if(argos.validation.check.postcode(input)) {
                                    // Call stores from postcode
                                    location = "StockResult_2.htm";
                                }
                                else {
                                    // Call town selection
                                    location = "StockResult_3.htm";
                                }
                                break;
            case "continueStore" : location = "StockResult_2.htm";
                              break;
            case "selectFave": // fall through.
            case "selectStore" : location = "";
                            break;*/
            default: ;// what?
        }
        
		return location ? _helpers.getUrlPathAndData(location) : location;
	}
    ...
    //    var activatee = argos.page.elements.checkStockActivatee;
     //   if(!activatee) argos.page.elements.checkStockActivatee = new argos.classes.CheckStockActivatee();
     //   argos.classes.TabbedArea($("#stockChecker .tab"), this.node);
    //    new argos.classes.CheckStockActivator($("#checkStock_go", this.node).get(0), activatee);
   //     new argos.classes.CheckStockActivator($("#continueStore", this.node).get(0), activatee);
   //     new argos.classes.CheckStockActivator($("#selectStore", this.node).get(0), activatee);
    //    new argos.classes.CheckStockActivator($("#selectFave", this.node).get(0), activatee);
		$("div.TabbedArea #stock2").hide();
		
		$("#emailMeSubmit", this.node).each(function() {
			new argos.classes.CheckStockActivator(this, argos.page.elements.checkStockActivatee);
		});
		
	}
    CheckStockActivatee.prototype.getStore = function() {
        var values = $("#storeSelection").serialize();
        var store = values.replace(/.*store=(\d+)/,"$1");
        return store != values ? store : null;
    }
    
    /* FIND STORE ACTIVATOR
	 ************************/
	_register(FindStoreActivator);
	function FindStoreActivator(activator, activatee) {
		LightboxActivator.apply(this, arguments);
		if(this.$node) {
			this.$node.unbind("click.activator");
			this.$node.bind("click.findStoreActivator", function(e) {
				e.preventDefault();
				var activator = this.Element;
				var activatee = activator.property("activatee");
				var location = activator.property("location" , activator.getLocation());
				...
				if(location) {
					...
					// You'll probably want to override close to do something with store value. 
					activatee.property("store", activatee.getStore());
					activatee.close();
				}
			});
		}
	}
	FindStoreActivator.prototype = _inherit(LightboxActivator);
	FindStoreActivator.prototype.constructor = FindStoreActivator;
	FindStoreActivator.prototype.intent = function(e) {
		var activatee = this.property("activatee");
		var location = this.property("location");
        activatee.property("ajax", $.ajax({
            url : location.path,
	...
	FindStoreActivator.prototype.getLocation = function() {
        var $form = this.$node.parents("form");
        var input = $("#stock01_OOPS", $form).val();
        var location;
        switch(this.node.id) {
            ...
            case "continueStore" : location = "StockResult_2.htm";
                              ...
                              break;
            case "selectFave": // fall through.
            case "selectStore" : location = null;
                            break;
            default: ;// what?
        }
	...
	/* FIND STORE ACTIVATEE
	 ************************/
	_register(FindStoreActivatee);
	function FindStoreActivatee(options) {
		LightboxActivatee.call(this, options);
	}
	FindStoreActivatee.prototype = _inherit(LightboxActivatee);
	FindStoreActivatee.prototype.constructor = FindStoreActivatee;
	FindStoreActivatee.prototype.modifyContent = function() {
        new argos.classes.TabbedArea($("#stockChecker .tab"), this.node);
        new argos.classes.FindStoreActivator($("#checkStock_go", this.node).get(0), this);
        new argos.classes.FindStoreActivator($("#continueStore", this.node).get(0), this);
        new argos.classes.FindStoreActivator($("#selectStore", this.node).get(0), this);
        new argos.classes.FindStoreActivator($("#selectFave", this.node).get(0), this);
	}
    FindStoreActivatee.prototype.getStore = function() {
        var values = $("#storeSelection").serialize();
        var store = values.replace(/.*store=(\d+)/,"$1");
        return store != values ? store : null;
    }
	...
	QvtActivatee._product = function(productItem) {
		var images = "images/";
		return {
			number :  productItem.partnumber?productItem.partnumber:"",
			title : productItem.description.replace(/\+/g, " "), // Manually replacing + symbols until encoding confirmed.
			quantity : productItem.quantity?parseInt(productItem.quantity,10):"",
			totalprice : productItem.totallineprice?parseFloat(productItem.totallineprice).toFixed(2):"",
			image : productItem.image ? argos.app.fileDir + images + productItem.image : argos.app.siteAssetsDir + images + "emptyProduct.gif"
		};
	}
	QvtActivatee._productLink = function(a, b) {
		return "<a href=\"/static/Product/partNumber/" + a + ".htm\">" + b + "</a>";
	};
	QvtActivatee.prototype.update = function() {
		var trolleyProp = argos.trolley.property;
		var link = this.constructor._productLink;
		var config = this.property("config");
		var displayLimit = config["displayLimit"] ? Number(config["displayLimit"]) : 5;
		var items = trolleyProp("products"); 
		var message = trolleyProp("message"); 
		var productCount = trolleyProp("totalitems");
		var itemCount = trolleyProp("itemCount");
		var cls = "progressive";
		var emptyClass="";
		var tag = "a";
		var p, html;
		if(items.length < 1) {
			...
			items = [{description:argos.app.qvt.emptyTrolley}];
			tag = "span";
			message = "";
			emptyClass = "trolleyEmpty";
		}
		...
		for(var i=0; i<items.length; ++i) {
			...
			if(i<displayLimit) {
				html += "<dl class=\"product "+emptyClass+"\">";
				html += "	<dd class=\"title\">";
				...
				html += "	</dd>"; 
				html += "	<dd class=\"price\">" + ((p.totalprice=="")?"":"<abbr lang=\"en\" class=\"currency\" title=\"GBP\">&pound;</abbr>"+p.totalprice) + "</dd>";  
				html += "</dl>";
			}
		}
		if(productCount > displayLimit) {
			html += "<" + tag + " class=\"trolleyLink more\">"; // Href injected via setLink().
			html += argos.app.qvt.moreProductsBar.replace("{{n}}",productCount - displayLimit) + ((productCount - displayLimit) > 1 ? "s" : "");
			html += "<span class=\"moreTotal\">&pound;" + trolleyProp("groupeditemstotalprice") + "</span></" + tag + ">"; 
		}
		if(!argos.trolley.isEmpty()){
			html += "	<div class=\"subtotal\">";
			html += "		<table><tr class=\"sub\"><th>"+argos.app.qvt.subtotal+"</th><td>&pound;"+trolleyProp("ordertotal")+"</td></tr>";
			if(Number(trolleyProp("totalorderadjustment"))!== 0){
				html += "<tr><th>"+argos.app.qvt.totalSpecialOffer+"</th><td>&pound;"+trolleyProp("totalorderadjustment")+"</td></tr>";
			}
			html += "	</table></div>";
		}
		...
		html += "		<" + tag + " class=\"button " + cls + " trolleyLink\">"+argos.app.qvt.trolleyButton+"</" + tag + ">"; // Href injected via setLink().
		html += "	</div>";
		html += "</div>";
		this.empty();
	...
	QvtActivatee.prototype.loadingElement = function() {
		var $ale = this.property("ajaxLoadingElement");
		if(!$ale) {
			// Not found as property so see if element exists.
			$ale = $("body > .ajaxLoadingFrame");
			if($ale.length < 1) {
				// Not created so do that now.
				$ale = $("<div class=\"ajaxLoadingFrame\">Loading your trolley...</div>").css("display", "none");
				$(document.body).append($ale);
			}
			$ale = $ale.clone().html("Loading your trolley...").css("display", "block");
			this.property("ajaxLoadingElement", $ale);
		}
		return $ale;
	}
	...
	function QvtActivator(activator, activatee) {
		...
		if(this.$node) {
			...
			this.$node.unbind("click.activator");
			var tlpLink = this.property("link");
			this.$node.click(function(e) {
				e.preventDefault();
				if(argos.trolley.getItemCount()!= 0){
					location.href = tlpLink;
				}
			});
			this.setCount();
		}
	...
	QvtActivator.prototype.intent = function() {
		...
		if(!activatee.property("active")) {
			activatee.property("active", true);
			if (argos.trolley.getItemCount() == 0 || (argos.trolley.hasContent() && !argos.trolley.hasExpired())) {
				activatee.update();
				activatee.setLink(activator.property("link"));
			}
			else {
				activatee.empty();
				activatee.loading(true);
				argos.trolley.update().done(function(){
					activatee.update();
					activatee.loading(false);
					activatee.setLink(activator.property("link"));
				});
	...
	QvtActivator.prototype.setCount = function() {	
		
		var theUrl = '/webapp/wcs/stores/servlet/ArgosUpdateCookies';
		var queryString = "storeId=" + argos.app.storeId + "&langId=" + argos.app.langId;
		var trolleyNode = this.$node;
		...
		$.ajax( {
			...
			success : function() { //this function is called when the ajax function is successfully executed		
				var counts = argos.page.user.getPersonalisation()["ti"];
				counts[0] = counts.length > 0 && Number(counts[0]) ? counts[0]  : 0;
				counts[1] = counts.length > 1 && Number(counts[1]) ? counts[1] : 0;
				var count = counts[1];
				if(trolleyNode) {
					trolleyNode.find(".count").text(count);
					if(count == 0){
						trolleyNode.removeClass("link");
					}else{
						trolleyNode.addClass("link");
					}
				}
			},
			error : function(xhr, ajaxOptions, thrownError) {
				//alert("ArgosUpdateCookies failed");
			}
		});
	}
...
//		argos.tracking.qvc.applyContinue.call(this.node);
	...
	RecognitionActivatee.prototype.addLoginFormToggle = function(context) {
		...
		$input.bind("click", function() {
			...
			if(this.checked && this.value != "registeruser") {
				$elements.show();
				if($("input[name='tabletCheckout']", context).val()){
					$('#registerUserFormEcom').attr("action", $("input[name='prefixHTTPS']", context).val()+"/AccessMyAccountMobile");
					$("input[name='logonIdFromTrolleyReg']", context).attr('name', 'logonId');
				}
			}
			else {
				$elements.hide();
				if($("input[name='tabletCheckout']", context).val()){
					$('#registerUserFormEcom').attr("action", $("input[name='prefixHTTPS']", context).val()+"/EditMyAccountMobile");
					$("input[name='logonId']", context).attr('name', 'logonIdFromTrolleyReg');
				}
			}
		});
	}
    ...
    function Trolley() {
    	Data.call(this);
    	this.property("itemCount", this.getItemCount());
    	this.property("message", "");
    	this.property("products", new Array());
    	this.property("groupeditemstotalprice", 0.00);
    	this.property("totalorderadjustment", 0.00);
    	this.property("ordertotal", 0.00);
    	this.property("empty", true);
    }
    Trolley.prototype = _inherit(Data);
    Trolley.prototype.constructor = Trolley;
    Trolley.prototype.isEmpty = function() {
    	return this.property("products").length < 1;
    }
    Trolley.prototype.update = function() {    	
    	var trolley = this;
		return $.ajax({
			url : "/webapp/wcs/stores/servlet/QuickViewTrolley",
			data : "langId=" + argos.app.langId + "&storeId=" + argos.app.storeId + "&rt=async",
			dataType : "json",
			type : "get",
			success : function(json) {
		    	if(!json.lines) {
		    		trolley.property("empty", true);
		    		//throw new ArgosComponentError("JSON request was successful but did not contain correct information to continue");
		    	}else{
		    		trolley.property("message", json.qvtmessage);
			    	trolley.property("groupeditemstotalprice", parseFloat(json.groupeditemstotalprice).toFixed(2));
			    	trolley.property("groupeditemsquantity", json.groupeditemsquantity);
			    	trolley.property("totalorderadjustment", parseFloat(json.totalorderadjustment).toFixed(2));
			    	trolley.property("ordertotal", parseFloat(json.ordertotal).toFixed(2));
			    	trolley.property("groupeditems", json.groupeditems);
			    	trolley.property("listeditems", json.listeditems);
			    	trolley.property("totalitems", json.totalitems);
			    	trolley.property("itemCount", trolley.getItemCount());
			    	
			    	// Update product Array.
			    	trolley.property("products", new Array());
			    	for(var i=0; i<json.lines.length; ++i) {
			    		if(json.lines[i]) { // Swallowing BE error that puts null value as line item.
				    		trolley.property("products").push({
				    			totallineprice : parseFloat(json.lines[i].totallineprice).toFixed(2),
				    			quantity : json.lines[i].quantity,
				    			image : json.lines[i].image,
				    			description : json.lines[i].description,
				    			partnumber : json.lines[i].partnumber
				    		});
			    		}
			    	}
			    	// Since we now have data.
			    	trolley.property("empty", false);
		    	}
				$(".QvtActivator").each(function() {
					this.Element.setCount();
				});
    ...
    Trolley.prototype.hasExpired = function() {
    	// Due to poor cookie organisation, we're using a User cookie.
		var expiryDateTime = argos.page.user.getPriceExpiry();
		var localTime = new Date();
		var localOffset = localTime.getTimezoneOffset()*6000;
		return (expiryDateTime.getTime() - localTime.getTime() + localOffset) <= 0;
    }
    Trolley.prototype.getItemCount = function() {
    	// Due to poor cookie organisation, we're using a User cookie.
    	return argos.page.user.getTrolleyCounts()[1];
    }
    Trolley.prototype.hasContent = function() {
    	return !this.property("empty");
    }

    /* ProductData
     **************/
    _register(ProductData);
    function ProductData() {
    	Data.call(this);
    }
    ProductData.prototype = _inherit(Data);
    ProductData.prototype.constructor = ProductData;
	...
	User.prototype.getPersonalisation = function() {
		...
		var personalisation = cookie ? {
			//{ti:[5621185|Sony Alpha A77 24MP 3D DSLR Camera with Lens - Black.|586-5621185IEUC1083825M.jpg|1,],1,1}{tim:[Have it all delivered from free,]}{rv:[],0}
			//ti : convertToArray.call(value, /.*?\{ti:\[(.*?),?\],?\d*,?\d*\}.*/),
			ti : convertToArray.call(value, /.*?\{ti:\[([,\d]*)\]\}.*/),
			...
			//expiry : value.replace(/.*?\{expiry:\[(.*?),?\]\}.*/,"$1"),
			expiry : convertToArray.call(value, /.*?\{expiry:\[([,\d]*)\]\}.*/),
			pc : convertToArray.call(value, /.*?\{pc:\[([,\d]*)\]\}.*/)
			}:{
	...
	User.prototype.convertPersonalisationToString = function(personalisation) {
		...
		var ti = "{ti:[" + personalisation["ti"] + "]}";
		var rv = "{rv:[" + personalisation["rv"].products  + "]," + personalisation["rv"].updated + "}";
		var expiry = "{expiry:["+personalisation["expiry"]+"]}";
		var pc = "{pc:["+personalisation["pc"]+"]}";
		return /*ti + tim +*/ti + rv+expiry+pc;
	}	
	User.prototype.getPersistentSession = function() {
		...
		var persistentSession = {
			...
			cssClass : value[2] ? value[2].toLowerCase() : "UNKNOWN",
			giftList : value[4],
			sessionId : value[5],
	...
	User.prototype.getTrolleyCounts = function() {
		...
		var counts = this.getPersonalisation()["ti"];
		counts[0] = counts.length > 0 && Number(counts[0]) ? counts[0]  : 0;
		counts[1] = counts.length > 1 && Number(counts[1]) ? counts[1] : 0;
		return counts;
	}
	User.prototype.getComparing = function() {
		// Return Array [totalProducts, totalItems]
		var partnumbers = this.getPersonalisation()["pc"];
		return partnumbers;
	}
	...
	User.prototype.getPriceExpiry = function() {
		// Returns Date object of PersonalisationCookie "expiry" value.
		return _helpers.dateFromString(this.getPersonalisation()["expiry"][1]);
	}
	User.prototype.setPriceExpiry = function(timestring) {
		var personalisation = this.getPersonalisation();
		personalisation.expiry[1] = timestring;
	...
	User.prototype.compareAdd = function(pn) {
		var personalisation = this.getPersonalisation();
		if ($.inArray(pn,personalisation.pc) == -1){
			personalisation.pc.push(pn);
			this.setPersonalisation(personalisation);
		}
	}
	User.prototype.compareRemove = function(pn) {
		var personalisation = this.getPersonalisation();
		personalisation.pc = $.grep(personalisation.pc, function(value) {
		  return value != pn;
		});
		this.setPersonalisation(personalisation);
	}
	User.prototype.compareClear = function() {
		var personalisation = this.getPersonalisation();
		personalisation.pc = [];
		this.setPersonalisation(personalisation);
	}
	...
	function QvpActivator(config) {
		...
		var url = argos.url.protocolChecked($product.find(".image a").attr("href"));
		...
		if(this.$node) {
			...
			if($container.length > 0) {
				...
				// The quick view box on the checkout page uses a different URL
				if(argos.page.elements['body'].is("#stockAvailability")) {
					this.property("url", url.replace("Product", "RelatedQuickView") + '?quantity=' + $product.find('.quickViewQuantity').text());
			}
		}
	}
	}
	...
	QvpActivator.prototype.intent = function() {
		...
		argos.tracking.qvp.activated.call(this.node);
	...
	function Expander(config) {
		Element.call(this, config ? config["container"] : null);
		var _expander = this;
		var _button = config && (config["openText"] || config["closeText"]) ? new Button({
			cssClass : "button",
		...
		if(this.node) {
			if(_button) { this.$node.append(_button.node) };
			this.property("config", config);
			this.intent();
		}
	}
	...
	Expander.prototype.intent = function() {
		this.toggle();
	}
	...
	Expander.prototype.open = function() {
		...
		var config = this.property("config");
		if(config) {
			if(button) button.$node.text(config["closeText"]);
	...
	Expander.prototype.close = function() {
		...
		var config = this.property("config");
		if(config) {
			if(button) button.$node.text(config["openText"]);
	...
	function Collection() {
		...
		// Can now replace var blah = new Collection(); assigments with var blah = $()
		// because we're using jQuery 1.7.2 + 
		// (OLD CODE) 
		//return jQuery.extend(this, (this.jquery < 1.4) ? $() : $().slice(1));
		// Now extended to act as a Collection validator/lookup.
		var $collection = $();
		if(arguments.length > 0 && arguments[0]) {
			if(_helpers.getType(arguments[0]) == "String") {
				$collection = $(arguments[0]);
			}
			if(arguments[0].jquery && arguments[0].length > 0) {
				$collection = arguments[0];
			}
		}
		return jQuery.extend(this, $collection);
	}
	...
	function ArgosLookupError(selector) {
		this.name = "ArgosLookupError";
		this.message = "An error has occurred trying to find the specified element.\nNot found: " + selector;
	}
	ArgosLookupError.prototype = new Error();
	ArgosLookupError.prototype.constructor = ArgosLookupError;
});